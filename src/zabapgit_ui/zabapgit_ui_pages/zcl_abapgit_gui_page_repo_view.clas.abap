...
CLASS ZCL_ABAPGIT_GUI_PAGE_REPO_VIEW IMPLEMENTATION.
  ...
  METHOD build_main_toolbar.
    ...
    " If user is not admin show only Stage by Transport (online repo) or keep existing minimal for offline
    IF zcl_abapgit_utils=>is_admin( ) = abap_false.
      IF mi_repo->is_offline( ) = abap_false.
        ro_toolbar->add( iv_txt = 'Stage by Transport'
                         iv_act = |{ zif_abapgit_definitions=>c_action-go_stage_transport }?key={ mv_key }|
      ...
      ELSE.
        " offline -> provide Export by Transport equivalent if desired, else nothing
        ro_toolbar->add( iv_txt = 'Export by Transport'
                         iv_act = |{ zif_abapgit_definitions=>c_action-zip_export_transport }?key={ mv_key }|
                         iv_opt = zif_abapgit_html=>c_html_opt-strong ).
      ENDIF.
      RETURN.
